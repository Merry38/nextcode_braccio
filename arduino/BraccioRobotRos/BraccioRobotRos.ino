...
Position initial_position(150, 90, 90, 90, 45,  GRIPPER_CLOSED);
...
void position_callback( const std_msgs::Float32MultiArray& position);
...
ros::Subscriber<std_msgs::Float32MultiArray> position_sub("position", position_callback);
...
void jointstates_callback( const sensor_msgs::JointState& joint)
{
    nh.loginfo("Joint callback invoked");
...
void position_callback( const std_msgs::Float32MultiArray& position)
{
    nh.loginfo("Position callback invoked");

    for(int i = 0; i < 6; i++)
    {
        req_servo_angle[i] = position.data[i] * 57.2958;
        pos[i] = position.data[i];
    }
    new_js_request = true;
}
